...
gui_spawn:
    ...
    Gui, Add, Button, x0 y0 w0 H0 Default , Google
    Gui, Show,, myGUI
    return
...
ButtonGoogle:
    ;~ Gui, Submit, NoHide
    ;~ gui_destroy()
    ;~ StringReplace, search_final_url, search_url%A_Index%, %Pederson%, %query_safe%
    ;~ run, %searcg_final_url%
    ;~ run %search_final_url%
    ;~ StringReplace, search_final_url, https://www.google.com/search?num=50&safe=off&site=&source=hp&q=REPLACEME&btnG=Search&oq=&gs_l=, REPLACEME, Pederson
    ;~ run %search_final_url%
    
    Gui, Submit
    gui_destroy()
    if !(ErrorLevel)  { 
        Pedersen := RegExReplace(RegExReplace(Pedersen, "\r?\n"," "), "(^\s+|\s+$)")
    If SubStr(Pedersen,1,7)="http://"
        Run, % Pedersen
    else 
        Run, % "http://www.google.com/search?hl=en&q=" Pedersen
  } 
